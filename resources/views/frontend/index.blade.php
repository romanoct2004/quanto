...
<body style="background-color: <?php echo $query->background_color; ?>; color: <?php echo $query->char_color; ?>; min-height: 100vh">
                ...
                <div class="row">
                    <div class="col brand">
                        <div class="brand-logo text-center">
                            @if ($query->profile_path)
                                @if (preg_match('/https?:\/{2}[\w\/:%#\$&\?\(\)~\.=\+\-]+/', $query->profile_path))
                                    <img src="{{ $query->profile_path }}" alt="brand-logo">
                                @else
                                    <img src="../{{ $query->profile_path }}" alt="brand-logo">
                                @endif
                            @endif
                            ...
                            <div class="col-6 col-lg-4 col-md-4 ">
                                <a class="pro-card load_content" data-survey="{{ $query->id }}" data-id="{{ $answer->id }}"
                                    ...
                                    <div class="image-holder">
                                        @if (preg_match('/https?:\/{2}[\w\/:%#\$&\?\(\)~\.=\+\-]+/', $answer->file_url ))
        ...
        <div class="row mx-3" style="position:fixed; bottom:50px; left:0; right:0;">
            ...
            <div class="col-2">
                ...
                <button class="btn btn-quanto px-4 px-sm-5 text-nowrap py-2 float-end ms-5" style="display:none;"
                    type="button" id="startBtn" onclick="firstTab()">注文手続きへ</button>
            </div>
        </div>
        ...
        <div class="total" id="total">
            ...
            <div class="row cart-item-list" id="item-list"
                ...
                <div class="col py-3">
                    ...
                    @isset($session_card)
                        @foreach ($session_card as $key => $each)
                            @php
                                $units = $units + $each['quantity'];
                            @endphp
                            <div class="row">
                                <div class="col-2">
                                    @if (preg_match('/https?:\/{2}[\w\/:%#\$&\?\(\)~\.=\+\-]+/', $each['img'] ))
    ...
    <script>
        $(document).ready(function() {
            ...
            $(".load_content").click(function() {
                ...
                if (isValidUrl($(this).data("image"))) {
                    imgSrc = $(this).data("image");
                }
                
                $('#total').hide();
                $('.product-image').html('').prepend(
                    '<img class="product-image-full image radius-top" src="' + imgSrc + '" />');
        ...
        $('#addToCart').on("submit", function(event) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    prevIndexes.push({
                        prevQuestionIndex: currentTab,
                        nextQuestionIndex: response.nextQuestionIndex,
                    });

                    if (response.nextQuestionIndex) {
                        $('#nextBtn').attr('onclick', `nextPrev(${response.nextQuestionIndex - currentTab})`); 
                        return;
                    }
    ...
    <script>
        ...
        function jump(n) {
            ...
            if (currentTab > n) {
                prevIndexes.pop();
                let prevBtnJumpIndex = 0;
                if (prevIndexes.length && prevIndexes[prevIndexes.length - 1].length !== 1) {
                    prevBtnJumpIndex = prevIndexes[prevIndexes.length - 1].prevQuestionIndex;
                }
                
                $('#nextBtn').attr('onclick', `nextPrev(1)`); 
                $('#prevBtn').attr('onclick', `jump(${prevBtnJumpIndex})`); 
            }
        ...
        function nextPrev(n) {
            ...
            if (n > 0 && prevIndexes[prevIndexes.length - 1].nextQuestionIndex) {
                $('#prevBtn').attr('onclick', `jump(${prevIndexes[prevIndexes.length - 1].prevQuestionIndex})`); 
            }
            if (n > 0 && !prevIndexes[prevIndexes.length - 1].nextQuestionIndex) {
