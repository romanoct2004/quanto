...
$(document).ready(function() {
    ...
    var request = $.get(url, function(data) {
        ...
        if (isValidUrl(data.brand_logo_path)) {
            $('#brand').css('background-image', 'url("' + data.brand_logo_path + '")');
        } else {
        ...
        if (isValidUrl(data.brand_logo_path)) {
            profile_img_url = data.user_profile_url;
        }
    ...
    if (n == 1) {
        ...
        if (qid_next != -1) {
            ...
            if (currentQuestion) {
                ...
                if (calculate) {
                    total += Number(currentAnswer.value)
...
function displaySurvey(data) {
    ...
    if (questionData.length > 0) {
        ...
        questionData.forEach(q => {
            ...
            if (q.file_url && isValidUrl(q.file_url)) {
                file_url = q.file_url;
            }
            if (q.file_url && !isValidUrl(q.file_url)) {
                file_url = `${serverHost}/${q.file_url}`;
            }
    ...
    if (answerList.length > 0){
        ...
        answers.forEach((ans) => {
            ...
            if (ans.type == TYPE.TEXT) {
                ...
                if (ans.file_url && isValidUrl(ans.file_url)) {
                    answerImage = `
                        <div class="answer-image" onclick="handleSelectAnswer(this, ${questionID}, ${ans.id}, 1)">
                        <img src="${ans.file_url}">
                        </div>
                    `;
                }
