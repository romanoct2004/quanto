...
function updateTextView(_obj) {
    ...
    if (num == 0) {
        ...
        _obj.attr('value', num.toLocaleString());
    }
}
...
function detail_make() {
    ...
    subHtml += `
        <div class="other-area">
            <textarea style="height: 100px; border: 1px solid grey; padding: 5px; box-sizing: border-box; font-size: 20px;" placeholder="(備考)" id="memo_text">${memo_text_val}</textarea>
            <div style="border: 1px solid grey; padding: 5px; box-sizing: border-box; margin-left: 10px; font-size: 20px; white-space: pre; display: none;" id="memo_text_for_print"></div>
            <div class="detail_price">`;
    subHtml += `<div><p>10%対象&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input class="sinput"id="totalAmount10" value="${$('#totalAmount10').val()}">円</p><p>&nbsp;&nbsp;消費税(内税)<input class="sinput"id="totalAmount10s" value="${$('#totalAmount10s').val()}">円</p></div>`;
    subHtml += `<div><p>8%(軽減税率)<input class="sinput"id="totalAmount88" value="${$('#totalAmount88').val()}">円</p><p>&nbsp;&nbsp;消費税(内税)<input class="sinput"id="totalAmount88s" value="${$('#totalAmount88s').val()}">円</p></div>`;
    subHtml += `<div><p>8%対象 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input class="sinput"id="totalAmount8" value="${$('#totalAmount8').val()}">円</p><p>&nbsp;&nbsp;消費税(内税)<input class="sinput" id="totalAmount8s" value="${$('#totalAmount8s').val()}">円</p></div></div></div>`;
    return subHtml;
}
...
async function mail_send_one(){
    ...
    const memoTextElm         = document.querySelector('#memo_text');
    const memoTextForPrintElm = document.querySelector('#memo_text_for_print');

    memoTextForPrintElm.innerHTML = memoTextElm.innerHTML;
    
    memoTextElm.style.display = 'none';
    memoTextForPrintElm.style.display = 'block';
    
    const formBody = document.getElementById("form_body").innerHTML;
    const option   = {
        margin: 1,
    ...
    fd.append('mails_text', mails_text);
    fd.append('mail_textarea', mail_textarea);
    fd.append('is_using_credit_card', is_using_credit_card);
    fd.append('total_price', parseInt($('#total_price').val().replaceAll(',', '')));
    fd.append('client_name', $('#uName').val());
    ...
    memoTextElm.style.display = 'block';
    memoTextForPrintElm.style.display = 'none';
}
...
$(document).ready(function () {
    ...
    $(document).on('click', '.tooltip-del', function () {
        ...
        inputIfChangeValueToInputAttrValue();
    });
    ...
    $(document).on('change', '[id^="price_"]', function () {
        ...
        console.log(parseInt($('#reduce_pro_' + mId).val()))
...
////////////////////////////////////////////////////////////
    $(document).on('click', '.img-upload-link-btn-1', function () {
        ...
        $('#price_' + current_img_index).val(display_img_price);
        $('#price_' + current_img_index).change();
...
$(function() {
    const set_div_toggles = document.querySelectorAll('.set-div-toggle');

    for (const set_div_toggle of set_div_toggles) {
        set_div_toggle.onchange = function() {
            is_using_credit_card = !is_using_credit_card;
            for (const set_div_toggle_for_onchange of set_div_toggles) {
                set_div_toggle_for_onchange.querySelector('input').checked = is_using_credit_card;
            }
        };
    }
});
...
const inputIfChangeValueToInputAttrValue = () => {
    for (const inputElm of document.querySelectorAll('#form_body input')) {
        inputElm.removeEventListener('change', inputChangeHandler)
        inputElm.addEventListener('change', inputChangeHandler)
        inputElm.setAttribute('value', inputElm.value);
    }

    for (const textAreaElm of document.querySelectorAll('#form_body textarea')) {
        textAreaElm.removeEventListener('change', textAreaChangeHandler)
        textAreaElm.addEventListener('change', textAreaChangeHandler)
        textAreaElm.innerHTML = textAreaElm.value;
        textAreaElm.setAttribute('value', textAreaElm.value);
    }
}