...
class SurveyController extends Controller
{
	...
	public function save(Request $request)
	{
		...
		if ($profile_file != null) {
			$extension = $profile_file->getClientOriginalExtension();
			...
			if (strtolower($extension) == 'png'
				...
				$profilePath = asset($profile_file->storeAs('public/survey', Str::uuid()));
				$profilePath = str_replace('public/survey', 'public/storage/survey', $profilePath);

				$survey->profile_path = $profilePath;
			}
		}
			...
			foreach ($q_keys as $key) {
				...
				if (isset($question_files[$key]['file_url'])) {
					...
					if ($question_file != null) {
						$extension = $question_file->getClientOriginalExtension();
						...
						if (strtolower($extension) == 'png'
							...
							$questionFilePath = asset($question_file->storeAs('public/question', Str::uuid()));
							$questionFilePath = str_replace('public/question', 'public/storage/question', $questionFilePath);
							$question->file_url = $questionFilePath;
							$question->save();
						}
					}
				}
						...
						if (isset($question_files[$key]['movie_file'])) {
							...
							if ($question_file != null) {
								...
								if (strtolower($extension) == 'mp4'
									|| strtolower($extension) == 'avi'
									|| strtolower($extension) == 'gif'
								) {
									$questionMoviePath = asset($question_file->storeAs('public/question/movie', Str::uuid()));
									$questionMoviePath = str_replace('public/question/movie', 'public/storage/question/movie', $questionMoviePath);
									$question->movie_file = $questionMoviePath;
									$question->save();
								}
							}
						}
				...
				if(isset($item['answers'])) {
					...
					foreach ($a_keys as $a_key){
						...
						if (isset($question_files[$key]['answers'][$a_key]['file_url'])) {
							...
							if ($answer_file != null) {
								$extension = $answer_file->getClientOriginalExtension();
								...
								if (strtolower($extension) == 'png'
									...
									$answerFilePath = asset($answer_file->storeAs('public/answer', Str::uuid()));
									$answerFilePath = str_replace('public/answer', 'public/storage/answer', $answerFilePath);
									$answerModel->file_url = $answerFilePath;
									$answerModel->save();
								}
							}
						}
