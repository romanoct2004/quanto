...
class frontendController extends Controller
{
    ...
    public function createCart(Request $request)
    {
        ...
        $answer = Answer::find($request->product_id);
        ...
        $session_cart = collect($session_cart)
            ->filter(function($session_cart_elm) {
                return intval($session_cart_elm['quantity']) > 0;
            })
            ->toArray();
        ...
        if ($answer !== null) {
            $questions = Question::where('survey_id', $request->survey_id)
                ->orderBy('ord')
                ->get();
    ...
    public function showPaymentInvoiceForm(Request $request) {
        if (!$request->has('id')) {
        ...
        Session::put('totalPrice', $invoice->total_price);

        return view('frontend.paymentInvoice', [
            'invoice' => $invoice,
        ]);
    }
    public function paymentInvoice(Request $request) {
        ...
        try {
            ...
            logs()->error($e);
            return back()->withInput()->with('error', 'カードが原因のエラーが発生しました。');
        } catch (\Stripe\Exception\InvalidRequestException $e) {
            logs()->error($e);
            return back()->withInput()->with('error', '正しいカード情報を入力してください。');
        } catch (\Exception $e) {
            logs()->error($e);
            return back()->withInput()->with('error', '不明なエラーが発生しました。時間をおいてからもう一度お試しください。');
        }
        ...
        PaidInvoice::create([
            ...
            'name' => $request->name,
            'kana' => $request->kana,
            'zip_code' => $request->postcodeFirst . $request->postcodeLast,
        ...
        return view('frontend.successPaymentInvoice')->with('customer_name', $invoice->to);
    }
}
